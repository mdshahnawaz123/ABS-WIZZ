...
namespace ABS_WIZZ.UI.AccessRequest
{
    public partial class AccessRequestWindow : Window
    {
        ...
        private void ComboDuration_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (ComboDuration.SelectedItem is ComboBoxItem selectedItem)
            {
                ...
                {
                    System.Windows.MessageBox.Show("Please write an email to bimdigitaldesign@gmail.com for more details regarding lifetime access.", 
        ...
        private async void BtnSubmit_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                System.Windows.MessageBox.Show("Please fill all the required fields.", "Input Error", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    if (result == MessageBoxResult.Yes)
                    {
                        try
                        {
                            string path = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "ABS_Wizz_Login_Details.txt");
                            System.IO.File.WriteAllText(path, $"ABS-Wizz Plugin Login Details\n=======================\nUsername: {username}\nEmail: {email}\nPassword: {password}\nPlan: {duration}\nRegistered: {DateTime.Now}");
                            System.Windows.MessageBox.Show($"Details saved to:\n{path}", "Saved", MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                        catch (Exception ex)
                        {
                            System.Windows.MessageBox.Show($"Could not save to Desktop: {ex.Message}", "Warning", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }
                    }
                ...
                {
                    System.Windows.MessageBox.Show($"There was an error submitting your request.\n\nDetails: {errorMessage}\n\nPlease try again later or contact support directly.", 
                                    "Submission Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
                System.Windows.MessageBox.Show($"An unexpected error occurred in the UI: {ex.Message}", 
                                "Submission Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
